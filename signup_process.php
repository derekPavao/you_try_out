...
/* These are the development public and private keys
$private_key = '6LcCbAsAAAAAAFO9xF2teH0XjDFpnd0IXZ9JzRVG';
$public_key = '6LcCbAsAAAAAABQW8TIX6rLWefbMQro67CoBpYAr';
*/

$public_key = '6Le1bAsAAAAAAJH7rqdK5XAqShio-rUl547yI3np';
$private_key = '6Le1bAsAAAAAAKvvzUEEDvdvHi3V2GCCxNcLnEFB';

// IN THE recaptcha_check_answer function below change the 'http://www.youtryout.com' parameter
// to $_SERVER['REMOTE_ADDR']
$resp = recaptcha_check_answer ($private_key, $_SERVER['REMOTE_ADDR'] , $_POST['recaptcha_challenge_field'], $_POST['recaptcha_response_field']);


if ($resp->is_valid && Validate::isEmail($_POST['email_address'])) {
	Flash::addMessage('Thank you for signing up for YouTryOut.com');
	header('Location: index.php');
} else {
	if (!$resp->is_valid){
		Flash::addMessage('You did not enter a valid captcha.');
	}
	if (!Validate::isEmail($_POST['email_address'])){
		Flash::addMessage('You must enter a valid email address.');
	}
	Flash::addMessage('Please try again');
?>
	...
	<input type="text" name="first_name" value="<?= $_POST['first_name'] ?>" onfocus="this.value=''" class="widebar" /><br />
	<input type="text" name="last_name" value="<?= $_POST['last_name'] ?>" onfocus="this.value=''" class="widebar" /><br />
	<input type="text" name="email_address" value="<?= $_POST['email_address'] ?>" onfocus="this.value=''" class="widebarbott" /><br />
